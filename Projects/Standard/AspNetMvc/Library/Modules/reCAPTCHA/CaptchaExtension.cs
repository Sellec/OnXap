...
namespace System.Web.Mvc
{
    public static class CaptchaExtension
    {
        public static IHtmlString RecaptchaInvisible(this HtmlHelper helper, string text, object htmlAttributes = null)
        {
            ...
            if (helper.ViewContext.Controller is OnXap.Core.Modules.ModuleControllerBase controller)
            {
                var appCore = controller.AppCore;
                if (appCore != null)
                {
                    var module = appCore.Get<OnXap.Modules.reCAPTCHA.ModuleReCaptcha>();
                    var cfg = module?.GetConfiguration<OnXap.Modules.reCAPTCHA.ModuleReCaptchaConfiguration>();
