        ...
        public override void Up()
        {
            if (!Schema.Table<RoleUser>().Exists())
            {
                Create.Table<RoleUser>().
                    WithColumn((RoleUser x) => x.IdRole).AsInt32().NotNullable().WithDefaultValue(0).
                    WithColumn((RoleUser x) => x.IdUser).AsInt32().NotNullable().WithDefaultValue(0).
                ...
                Create.PrimaryKey("RoleUserKey").OnTable(GetTableName<RoleUser>()).Columns(
                    GetColumnName((RoleUser x) => x.IdRole),
                    GetColumnName((RoleUser x) => x.IdUser)
                );
            }
            else
            {
                AddColumnIfNotExists(Schema, (RoleUser x) => x.IdRole, x => x.AsInt32().NotNullable().WithDefaultValue(0));
                AddColumnIfNotExists(Schema, (RoleUser x) => x.IdUser, x => x.AsInt32().NotNullable().WithDefaultValue(0));
