...
namespace OnXap.CoreBind._Initialize
{
        ...
        public static void Initialize() 
        {
            ...
            try
            {
                ...
                if (System.Diagnostics.Process.GetCurrentProcess().ProcessName == "Microsoft.VisualStudio.Web.Host")
                {
                    ...
                    if (Debug.IsDeveloper) Debug.WriteLine($"{typeof(OnXap.NamespaceAnchor).Name}.Start: managerInjectorType='{managerInjectorType?.ToString()}'");
                        ...
                        if (launchMethod != null && loadAssemblyMethod != null)
                        {
                            ...
                            foreach (var process in devEnvProcesses)
                            {
                                try
                                {
                                    ...
                                    //    catch (Exception ex)
                                    //    {
                                    //        Debug.WriteLine($"{typeof(OnXap.NamespaceAnchor).Name}.Start: process '{process.ProcessName}' (pid {process.Id}), error loading web assembly - '{webAssembly.FullName}'");
                                    //    }
                                    //}
                                    ...
                                    Debug.WriteLine($"{typeof(OnXap.NamespaceAnchor).Name}.Start: process '{process.ProcessName}' (pid {process.Id}), success");
                                }
                                catch (Exception ex)
                                {
                                    Debug.WriteLine(typeof(OnXap.NamespaceAnchor).Name + ".Start: process '{0}' (pid {1}), error: {2}", process.ProcessName, process.Id, ex.ToString());
                                }
                            }
                        }
                        else if (launchMethod == null && loadAssemblyMethod == null && Debug.IsDeveloper) Debug.WriteLine($"{typeof(OnXap.NamespaceAnchor).Name}.Start: launch && loadAssemblyMethod method not found");
                        else if (launchMethod == null && Debug.IsDeveloper) Debug.WriteLine($"{typeof(OnXap.NamespaceAnchor).Name}.Start: launch method not found");
                        else if (loadAssemblyMethod == null && Debug.IsDeveloper) Debug.WriteLine($"{typeof(OnXap.NamespaceAnchor).Name}.Start: loadAssemblyMethod method not found");
                    }
                    else if (Debug.IsDeveloper) Debug.WriteLine($"{typeof(OnXap.NamespaceAnchor).Name}.Start: not managerInjectorType");
                }
            }
            catch (Exception ex)
            {
